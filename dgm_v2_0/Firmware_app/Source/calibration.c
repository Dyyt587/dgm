        ...
        case CS_DIR_PP_END:
            {
                ...
                {
                    uint8_t data[4];
                    int32_to_data(UsrConfig.encoder_dir, data);
                    CAN_calib_report(3, data);
                }
                
                // Motor pole pairs
                UsrConfig.motor_pole_pairs = round(2.0f / ABS(diff/ENCODER_CPR_F));
                
                {
                    uint8_t data[4];
                    int32_to_data(UsrConfig.motor_pole_pairs, data);
        ...
        case CS_ENCODER_END:
            {
                ...
                {
                    uint8_t data[4];
                    int32_to_data(UsrConfig.encoder_offset, data);
        ...
        case CS_REPORT_OFFSET_LUT:
            if(sample_count < OFFSET_LUT_NUM){
                if(time > next_sample_time){
                    ...
                    {
                        uint8_t data[4];
                        int32_to_data(UsrConfig.offset_lut[sample_count], data);
